...
# forked from https://github.com/ly0/baidupcsapi/commit/9bca4cb38131d92039aaaf8c2257edc47cb6e2f0
# since the upstream does not support python2 any more
...
'''
logging.basicConfig(filename='/tmp/dupan.log',level=logging.DEBUG, format='%(asctime)s %(filename)s[line:%(lineno)d] %(levelname)s %(message)s', datefmt='%a, %d %b %Y %H:%M:%S')
'''
...
class BaseClass(object):
    ...
    def _check_account_exception(self, content):
        ...
        if err_id == '120021':
            ...
            if resp.ok:
                # get vcode
                vcode = self.verify_func('Verification Code> ')
